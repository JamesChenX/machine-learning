...
* **尽管ML代码由于涉及很多数学公式，因此可能很复杂，但另一方面，数学公式是很固定的，且ML的工程实践+代码又比较模板化，因此结合LLM，可以极大地帮助写代码。**
  **初学ML的过程中，更推荐重理解，轻代码（比如用代码手写数学公式，很容易卡死。数学大佬随意）。等之后真实践了，让LLM帮忙写代码，自己看得懂，能理解就行。**
  ...
  另外，在LLM时代，单从开发角度来说，开发任意类型项目的难点在于：1. 公司组织架构（注意：不是项目架构，而是整个公司的组织架构），公司的组织架构决定公司整体的技术架构，进而决定项目的技术架构=>开发栈+功能设计等等；2. 功能模块的tokens总数（如果功能模块只有几K行代码，那可以很轻松地把它输入给LLM，让它增删改功能，基本全靠自动挡；而如果功能模块有几十万行代码）。P.S. 1~2K行代码，通常就有1~3W tokens了，所以非常不耐用。 
...
假设你要训练一个模型预测房价，输入特征包括：
* 房屋面积（平方米）：50 ~ 200
* 房间数量（个）：1 ~ 5

如果直接用原始数据：
* 面积（200）的影响远大于房间数（5），模型会过度关注面积，忽略房间数的作用。
* 优化器需要小心翼翼地调整学习率，否则可能因数值差异过大而无法收敛。

归一化后（如缩放到 0~1）：

* 面积：0.25 ~ 1.0
* 房间数：0.2 ~ 1.0
* 所有特征对模型的影响权重相当，训练更高效。

##### 常见方法

| 方法                        | 公式                                                                                   | 作用                                                                      | 适用场景                                     | 代码示例                                                                                                                                                                       |
| ------------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Min-Max（0~1 缩放）**       | ![](Pasted%20image%2020250411223804.png)                                             | 将数据线性缩放到 [0, 1] 区间                                                      | 图像像素、数据范围明确（如温度、年龄）                      | data = np.array([50, 100, 150, 200])<br>normalized = (data - data.min()) / (data.max() - data.min())                                                                       |
| **Z-Score（均值方差归一化）**      | ![](Pasted%20image%2020250411223811.png)<br>![](Pasted%20image%2020250411224222.png) | 将数据转换为均值为 0、标准差为 1 的分布                                                  | 数据符合正态分布（如身高、考试成绩）<br><br>异常值较少的情况       | data = np.array([1, 2, 3, 4, 5])<br>mean, std = data.mean(), data.std()<br>standardized = (data - mean) / std                                                              |
| **Robust Scaling（鲁棒归一化）** | ![](Pasted%20image%2020250411223820.png)![](Pasted%20image%2020250411224243.png)     | 对异常值不敏感，适用于数据中有极端值的情况                                                   | 含异常值的数据（如：- 收入数据（少数人收入极高）；传感器数据（偶尔出现噪声）） | from scipy.stats import iqr<br>data = np.array([1, 2, 3, 4, 100])  # 含异常值 100<br>median, iqr_val = np.median(data), iqr(data)<br>robust_scaled = (data - median) / iqr_val |
| **Batch Norm（批归一化）**      | ![](Pasted%20image%2020250411224418.png)![](Pasted%20image%2020250412113650.png)     | 1. 将每个特征调整为均值为0、方差为1。<br>2. 在神经网络的每一层输入前进行归一化，防止梯度消失/爆炸                 | 深度神经网络中间层                                | bn = nn.BatchNorm1d(100)                                                                                                                                                   |
| **Layer Norm（层归一化）**      |                                                                                      | 对单样本所有特征归一化。<br><br>与BN的区别：BN 对 batch 内所有样本的同一特征归一化，而 LN 对单个样本的所有特征归一化。 | RNN、Transformer                          | ln = nn.LayerNorm(100)                                                                                                                                                     |
##### Batch Normalization

原始值（蓝点）在归一化之后，以零（红点）为中心。这确保了所有特征值现在都处于同一尺度。

![](Pasted%20image%2020250412200630.png)
...
![](Pasted%20image%2020250412201134.png)

Batch Norm 层的放置顺序

关于 Batch Norm 层在架构中的位置，目前有两种观点：

* 放置在神经网络层间，而非对原始数据中进行归一化。
* 激活函数之前或之后，主流是用于激活函数之前。

![](Pasted%20image%2020250412201253.png)

注意事项：

* 训练和测试模式不同：
	* 训练时：使用当前批次的均值和方差。
	* 测试时：使用全局统计量（移动平均的均值和方差）。
* 不适用于小批次（Batch Size < 16）：
	* 小批次的统计估计不准确，可能反而降低性能。

##### 类别——硬归一化与软归一化（Hard Normalization & Soft Normalization）

| 特性        | Hard Normalization                                                                             | Soft Normalization（如 Min-Max/Z-Score）         |
| --------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------- |
| 核心思想      | 通过阈值化、二值化或分段函数，将输入数据强行压缩到一个固定范围（如 {0, 1} 或 {-1, 1}）                                            |                                               |
| **映射方式**  | 非线性（阈值、离散化）                                                                                    | 线性缩放                                          |
| **保留信息**  | 部分丢失                                                                                           | 保留原始分布                                        |
| **抗噪声能力** | 强                                                                                              | 弱                                             |
| 适用场景      | - 数据本身具有离散性（如分类标签）。<br>- 需要抗噪声或极端值（如医疗图像）。  <br>- 资源受限（如嵌入式设备）。                                | - 需要保留原始数据分布（如回归任务）。<br>- 数据连续且无显著异常值（如房价预测）。 |
| **典型应用**  | 二值图像、离散数据                                                                                      | 连续特征（如图像像素、传感器数据）                             |
| 优点        | 抗噪声能力强：通过离散化或截断，减少异常值的影响。<br>计算高效：二值化后的数据可加速矩阵运算（如二进制神经网络）。<br>模型简化：适合低精度计算（如边缘设备上的 1-bit 模型）。 |                                               |
| 缺点        | 信息丢失：丢弃了原始数据的细节，可能影响模型精度。<br>阈值敏感：人工设定的阈值可能不普适，需调参。                                            |                                               |

##### 注意事项

* 训练集和测试集必须用相同的归一化参数（如均值、标准差），否则会引入偏差。
* 图像数据通常只需 /255，因为像素值天然在 [0, 255] 之间。
* 树模型（如随机森林、XGBoost）通常不需要归一化，因为它们基于特征划分，不受数值范围影响。

#### 过拟合（Overfitting）

定义：模型过度学习了训练数据中的噪声和细节（而非通用规律），导致泛化能力差。

现象（判断方法）：
* 训练集准确率高（如99%），测试集准确率低（如60%）。
* 训练Loss持续下降，但验证Loss先降后升。

原因：
* 模型太复杂：比如神经网络层数过多、参数过多（相当于“死记硬背”）。
* 数据太少或噪声多：模型容易记住噪声而非规律。
* 训练时间过长：模型逐渐拟合训练数据中的无关细节（类似“钻牛角尖”）。

![](Pasted%20image%2020250413093459.png)

##### 为什么要关注大权重

| 问题        | 现象                          | 原因                                               | 例子                                                   | 正则化的作用        |
| --------- | --------------------------- | ------------------------------------------------ | ---------------------------------------------------- | ------------- |
| **过拟合**   | 模型在训练集上表现极好，但在测试集上表现差       | 大权重意味着模型过度依赖某些特征（甚至噪声），导致决策边界过于复杂                | 假设用高阶多项式拟合数据，某些特征的权重极大，模型会强行穿过所有训练数据点（包括噪声），但泛化能力极差。 | 限制权重，使模型更简单   |
| **梯度爆炸**  | 训练时Loss剧烈震荡甚至变成NaN          | 大权重会导致反向传播时梯度连乘后变得极大（例如LSTM中梯度指数增长），优化器无法稳定更新参数。 |                                                      | 防止反向传播时梯度连锁放大 |
| **输入敏感**  | 输入数据的微小变化（如像素值±1）可能导致输出剧烈波动 | 大权重放大了输入的变化（例如：y = 1000*x，若x变化0.1，y变化100）。       |                                                      | 降低模型对噪声的敏感性   |
| **特征不平衡** | 模型过度依赖某些特征，忽略其他重要特征         |                                                  | 在房价预测中，若“房屋面积”的权重极大，而“房间数量”的权重极小，模型会几乎忽略后者           | 强制模型均衡利用所有特征  |
##### 解决办法

1. 增加训练数据
	* 核心思想：让模型见多识广，避免“以偏概全”。
	* 实操：收集更多数据，或通过数据增强（如旋转、裁剪图像）生成伪数据。
2. 简化模型
    * 减少参数量：降低神经网络层数、神经元数量。
    * 改用简单模型：如用线性回归代替多项式回归。
3. 正则化（Regularization）
	* 核心思想：限制模型的自由度（如权重大小、神经元依赖），迫使它学习通用规律而非训练数据的噪声和细节。
	* 惩罚大权重的原因：大权重意味着模型过度依赖某些特征（甚至噪声），导致决策边界过于复杂。
	* 为什么限制权重有效：将权重看作弹簧，L2正则化相当于给弹簧增加阻力（拉力越大，反弹力越强），防止某些弹簧被拉得过长（大权重）。
    * L1/L2正则化：在损失函数中增加权重惩罚项，惩罚大权重，迫使模型参数变小。
	    * L1（Lasso。稀疏化）：惩罚权重的绝对值之和，倾向于产生稀疏权重（部分特征被彻底忽略）。
	      ![](Pasted%20image%2020250413113717.png)
	      公式的第二部分是模型权重值的绝对和。
	      注意，L1正则化通过将它们合并到损失值计算中，确保对权重的高绝对值进行惩罚
	      
	      效果：
		      * 直接消除不重要的特征（权重为0），相当于自动特征选择。
		      * 适合高维数据（如文本分类）。
	    * L2（Ridge；更常用；权重衰减）：惩罚权重的平方和，倾向于让权重均匀减小。
	      ![](Pasted%20image%2020250413113710.png)
	      ```python
...
| 权重分布     | 无正则化    | L2正则化  | L1正则化    |
| -------- | ------- | ------ | -------- |
| **特征1**  | 100.0   | 1.2    | 0.0（被丢弃） |
| **特征2**  | 0.01    | 0.8    | 0.5      |
| **模型行为** | 完全依赖特征1 | 平衡使用特征 | 仅用特征2    |

4. Dropout
   随机丢弃神经元：训练时每次随机关闭一部分神经元（如比例设为0.5），在下一个迭代中另一部分随机的神经元被关闭，防止过度依赖某些特征。

![](Pasted%20image%2020250413113816.png)

```python
model = nn.Sequential(
    nn.Linear(100, 50),
    nn.Dropout(0.5),  # 50%的神经元被随机置零
    nn.ReLU()
)
```
注意：在预测时，不需要应用dropout。在`model.eval()`评估模式下，dropout层不会生效。

5. 早停（Early Stopping）
   监控验证集Loss：当验证集Loss不再下降时停止训练，避免过度拟合。
```python
from pytorchtools import EarlyStopping
early_stopping = EarlyStopping(patience=5)  # 容忍5次验证Loss不下降
```

6. 交叉验证（Cross-Validation）
   数据分多份轮换训练：如5折交叉验证，充分利用数据并评估泛化能力。

7. Batch Normalization
   标准化层间输入：减轻内部协变量偏移，间接降低过拟合风险。

8. 集成学习（Ensemble）
   “三个臭皮匠顶个诸葛亮”：
	   * Bagging：如随机森林（多个模型投票）。
	   * Boosting：如XGBoost（逐步修正错误）。
...
10. 迁移学习（Transfer Learning）
    借用预训练模型：如用ResNet提取图像特征，仅微调最后几层，避免从头训练。
...
GNN的核心是通过消息传递机制将图结构的拓扑信息与节点/边特征深度融合。其设计需权衡：

* 局部性（邻居聚合的深度与广度）
* 表达能力（聚合函数的复杂度）
* 计算效率（采样与并行化策略）

理解GNN的关键是：它并非单一模型，而是一套针对图数据的特征工程与表示学习框架。
## 核心思想：消息传递（Message Passing）

GNN的所有变体都围绕这一范式展开，其数学本质是**局部拓扑结构的特征聚合**。分为三个阶段：

1. 消息生成（Message）
   ![](Pasted%20image%2020250414213025.png)
2. 消息聚合（Aggregation）
   ![](Pasted%20image%2020250414213038.png)
3. 节点更新（Update）
   ![](Pasted%20image%2020250414213049.png)
## 多层GNN与感受野

* 堆叠层数：每增加一层GNN，节点能接收到的信息范围扩展一跳（Hop）。
	* 例如：2层GNN可捕获“朋友的朋友”的影响。
* 过平滑问题：深层GNN可能导致所有节点特征趋同（因过度信息混合）。解决方案：
...
|模型|核心创新|适用场景|
|---|---|---|
|GCN|谱域卷积的局部一阶近似|同构图，中等规模图|
|GraphSAGE|邻居采样+多种聚合方式|大规模图，动态图|
|GAT|注意力机制加权聚合|异构图，关键关系挖掘|
|GIN|理论证明最强的表达能力（WL-test）|图分类等需高区分度的任务|
## 难点与解决方案

1. 邻居爆炸问题（Neighbor Explosion）
问题：节点度数高时（如社交网络大V），聚合计算成本剧增。
...
2. 异构图处理
问题：节点/边类型多样（如电商图中用户、商品、购买行为）。

解法：
* 元路径（Meta-Path）设计（HAN）
* 关系图卷积（R-GCN）
...
![](Pasted%20image%2020250414213416.png)
...
* 难以处理超大规模图（如数十亿节点）。
* 对长程依赖（Long-range Dependency）建模能力弱。

前沿方向：
* 图Transformer（如Graphormer）
* 自监督图学习（如对比学习）
* 图结构学习（Joint Learning of Graph and Features）
# 提示词工程
